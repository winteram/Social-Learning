...
def move(roundsalive,myrepertoire,history_R,history_M,history_A, history_P, historyDemes, currentDeme, canChooseModel, canPlayRefine, multipleDemes):
#def discountmachine(history_R, history_M, history_A, history_P, roundsalive, myrepertoire):
	...
	for key, value in history_R.items():
		historyR[key] = value
	...
	myhistory = np.rowstack((historyR, historyM, historyA, historyP))
	...
	if roundsalive == 0:
		move = 0
	elif roundsalive == 1 and not myrepertoire:
		move = -1
	else:
		if myhistory.size:
...
def estimatebestmoveandpayoff(roundsalive, myhistory, myrepertoire, hatpsubc, hatmean):
	...
	for idx, val in enumerate(myrepertoire[0]):
		...
		if pertinent[1,len(pertinent[1,:])-1] == 0:
			...
			myrepertoire[1,idx] = pertinent[3,len(pertinent[3,:])-1]
...
def linebuddy(myhistory, hatpsubc, scalefactor):
	...
	for idx in range(0, sum(obsindices)):
