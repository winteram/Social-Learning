...
def test(inputstring):
    return "Other agent received "+inputstring
...
#'This function MUST return a tuple in the form (MOVE,ACT) if MOVE is EXPLOIT or REFINE, or (MOVE,) if MOVE is INNOVATE or OBSERVE'
    if roundsAlive>1: #if this isn't my first or second round
        ...
        if lastMove==OBSERVE: #if lastMove was observe or lastPayoff at least as good as myMeanPayoff
            if random.random()<0.25 and canChooseModel: #if simulation allows refinement
