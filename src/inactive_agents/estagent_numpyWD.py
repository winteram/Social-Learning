...
def move(roundsAlive,rep,historyRounds,historyMoves,historyActs, historyPayoffs, historyDemes, currentDeme, canChooseModel, canPlayRefine, multipleDemes):
    ...
    def estimate_payoff(CurrMax, E_lifeleft, DistName, params) :
        ...
        if DistName == 'Expon' :
            ...
            (loc, lamb) = params
            i = 1
            EstPos_temp = PL_cdf(CurrMax + i + Delta, lamb) - PL_cdf(CurrMax + i - Delta, lamb)
            while EstPos_temp > epsilon:
                EstPos_temp = PL_cdf(CurrMax + i + Delta, lamb) - PL_cdf(CurrMax + i - Delta, lamb)
                # for continuous distribution, P[X=x] is calculated as Cdf[x+0.5] - Cdf[x-0.5]. 
                EstPayoff_temp = EstPos_temp * i
    ...
    def est_fit(values):
        ...
        x = np.asarray(range(1,len(counts)+1))
        ...
        exp_params = np.linalg.lstsq(A, logy)[0]
        exp_SSE = [(logy[j] - exp_params[1] - 1.5*exp_params[0]*xvals[j])**2 for j, i in enumerate(xvals)]

        # pl fit
        logx = np.log(xvals)
        B = np.vstack([logx, np.ones(len(xvals))]).T
        pl_params = np.linalg.lstsq(B, logy)[0]
        pl_SSE = [(logy[j] - pl_params[1] - pl_params[0]*logx[j])**2 for j, i in enumerate(logx)]

        if (np.mean(exp_SSE) < np.mean(pl_SSE)):
            return("Expon", (exp_params[1],-1.5*exp_params[0]))
        else:
            return("PL", (pl_params[1],pl_params[0]))
    ...
    if roundsAlive==1 and len(rep.items())==0:
        ...
        if len(rep.items()) < ACTTHRESH or np.std(rep.values())==0:
            ...
            model, params = est_fit(rep.values())
            if params[1] < ofl_epsilon:
                return (OBSERVE,)
            exp_decision = estimate_payoff(max(rep.values()),max(EXPECTED_LIFETIME-roundsAlive,10),model,params)
            if exp_decision == 1:
                act = sorted(rep, key=rep.get, reverse=True)[0]
                return (EXPLOIT,act)
