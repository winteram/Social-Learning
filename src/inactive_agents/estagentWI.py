...
#        exec('del ' + obj)
    ...
    def vecnorm(x, ord=2):
        if ord == Inf:
            ...
            return np.amin(abs(x))
        else:
            return np.sum(abs(x)**ord,axis=0)**(1.0/ord)
    ...
    def fmin_cg(f, x0, fprime=None, args=(), gtol=1e-5, norm=Inf, epsilon=_epsilon,
        ...
        while (gnorm > gtol) and (k < maxiter):
            ...
            if gfkp1 is None:
                gfkp1 = myfprime(xk)
            yk = gfkp1 - gfk
            beta_k = pymax(0,np.dot(yk,gfkp1)/deltak)
            pk = -gfkp1 + beta_k * pk
            gfk = gfkp1
            gnorm = vecnorm(gfk,ord=norm)
            if callback is not None:
...
################  END SciPy FUNCTIONS ###########################
    ...
    def estimate_payoff(CurrMax, E_lifeleft, DistName, params, canPlayRefine=False) :
        Delta = 0.5
        ErrorFlag = 0
        EstPayoff_total = 0
        ...
        if (EstPayoff_total > CurrPayoff) and  (ErrorFlag == 0):
            ...
            Decision_result = 0
            if (EstPayoff_total <= CurrPayoff + 1) and canPlayRefine==True:
                Decision_result = 2
    ...
    if roundsAlive==1 and len(rep.items())==0:
        ...
        if len(rep.items()) < 10 or np.std(rep.values())==0:
            ...
            exp_decision = estimate_payoff(max(rep.values()),max(EXPECTED_LIFETIME-roundsAlive,10),'Expon',params,canPlayRefine)
            if exp_decision == 1:
                act = sorted(rep, key=rep.get, reverse=True)[0]
                return(EXPLOIT,act)
            elif exp_decision == 2:
